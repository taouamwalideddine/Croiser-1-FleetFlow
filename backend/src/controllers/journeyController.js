...
// Check if a truck is available for a journey
...
export const updateJourneyStatus = async (req, res, next) => {
  try {
    ...
    journey.logs.push({
      status,
      note: note || `Statut mis à jour à: ${status}`,
      timestamp: new Date(),
    ...
    // If journey was in progress and status is changing
